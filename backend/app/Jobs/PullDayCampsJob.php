...
class PullDayCampsJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(CrmSyncService $syncService): void
    {
        $syncService->sync([
            'resource'      => 'day_camps',
            'endpoint'      => '/CrmToMobileSync/getDayCampListData',
            'model'         => DayCamp::class,
            'primaryKey'    => 'crm_id',
            ...
            'fieldMap' => function (array $r) use ($syncService): array {
                return [
                    ...
                    'turnus_name'         => (string) ($r['turnusName']             ?? ''),
                    'departure_place'     => (string) ($r['departurePlace']         ?? ''),
                    'transport_options'   => json_encode($r['transportOptions']     ?? []),
                    'diet_options'        => json_encode($r['dietOptions']          ?? []),
                    'medical_required'    => (int)    ($r['medicalRequired']        ?? 0),
                    'guardian_required'   => (int)    ($r['guardianRequired']       ?? 0),
