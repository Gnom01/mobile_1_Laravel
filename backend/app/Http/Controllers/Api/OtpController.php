...
class OtpController
{
    ...
    public function requestOtp(Request $request, SerwerSmsClient $sms)
    {
        ...
        $phone = $this->normalizeToE164($request->string('phone'));
    ...
    /**
     * Normalize phone number to E.164 format (+48...).
     * Handles: "+48123456789", "48123456789", "123456789", "123 456 789"
     */
    private function normalizeToE164(string $phone): string
    {
        ...
        // Starts with 48 (without +)
        if (str_starts_with($phone, '48') && strlen($phone) === 11) {
            return '+' . $phone;
        }

        // Just 9 digits (Polish mobile)
        if (preg_match('/^\d{9}$/', $phone)) {
            return '+48' . $phone;
        }

        // Already has other country prefix
